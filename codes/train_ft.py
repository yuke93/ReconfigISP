...
def main():
    ...
    # proxy fine-tuning
    ft_interval = opt['proxy_ft_params']['ft_interval']
    ...
    while current_step <= total_iters:
        ...
        for train_data, train_val_data in zip(train_loader, train_val_loader):
            ...
            # optimization
            assert 'ft' in model_type
            if model_type == 'darts_ft':
                if current_step % ft_interval == 0:
                    model.finetune_proxies()
                    if rank <= 0:
                        print('proxy nets fine-tuned!')
