...
save_dir = '/DATA/SID/imgs/SID_Sony_Test_Rggb2Bgr/'
if not osp.exists(save_dir):
    os.mkdir(save_dir)

# get train IDs
train_fns = glob.glob(osp.join(gt_dir, '1*.ARW'))  # bug fixed, should use gt_dir rather than input dir
print(len(train_fns))
...
for id in train_ids:
    ...
    in_files = glob.glob(osp.join(input_dir, '{:05d}'.format(id) + '_00*.ARW'))
    gt_path = glob.glob(osp.join(gt_dir, '{:05d}'.format(id) + '_00*.ARW'))[0]  # the same GT for different inputs
    ...
    for k, in_path in enumerate(in_files):
        ...
        # save whole image
        # noisy raw
        key_noisy = osp.splitext(in_fn)[0] + '_noisy.png'
        keys_noisy.append(key_noisy)
        name_noisy = osp.join(save_dir, key_noisy)
        cv2.imwrite(name_noisy, raw_noisy)

        # ratio raw
        key_ratio = osp.splitext(in_fn)[0] + '_ratio.png'
        keys_ratio.append(key_ratio)
        name_ratio = osp.join(save_dir, key_ratio)
        cv2.imwrite(name_ratio, raw_ratio)

        # gt RGB
        key_gt = osp.splitext(gt_fn)[0] + '_gt.png'
        keys_gt.append(key_gt)
        if k == 0:
            name_gt = osp.join(save_dir, key_gt)
            cv2.imwrite(name_gt, gt_bgr)
