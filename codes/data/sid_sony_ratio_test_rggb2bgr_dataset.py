...
class SidSonyRatioTestRggb2BgrDataset(data.Dataset):
    '''
    ...
    def __init__(self, opt):
        ...
        wrong_ids = ['10034', '10045', '10172']
        self.keys_noisy = [key for key in self.keys_noisy if key[:5] not in wrong_ids]
        self.keys_gt = [key for key in self.keys_gt if key[:5] not in wrong_ids]

        # data type
        # normally, we do not use lmdb for SID test sets
        ...
        if self.sid_expo_in is None and self.sid_expo_gt is None:
            ...
            keys_noisy = [k_in for k_in, k_gt in zip(self.keys_noisy, self.keys_gt) if
                          self.sid_expo_in in k_in and self.sid_expo_gt in k_gt]
            keys_gt = [k_gt for k_in, k_gt in zip(self.keys_noisy, self.keys_gt) if
                       self.sid_expo_in in k_in and self.sid_expo_gt in k_gt]
    ...
    def __getitem__(self, index):
        ...
        if data_size is not None:
            img_noi = img_noi[:data_size, :data_size, :]
            img_gt = img_gt[:data_size, :data_size, :]
